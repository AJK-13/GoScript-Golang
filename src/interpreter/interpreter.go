...
func Eval(node ast.Node, environment *env.Environment, res semantic.Resolution) (interface{}, error) {
	...
	case *ast.Print:
		...
		fmt.Fprint(options.Writer, value)
		return nil, nil
	case *ast.Println:
		value, err := Eval(n.Expression, environment, res)
		if err != nil {
	...
	case *ast.Final:
		if n.Initializer != nil {
			...
			environment.Define(n.Name.Lexeme, value, n.EnvIndex, "final")
		} else {
			environment.DefineUnitialized(n.Name.Lexeme, n.EnvIndex)
		}
		return nil, nil
	...
	case *ast.Call:
		...
		if function.Arity() != len(args) {
			return nil, runtimeerror.Make(n.Paren, fmt.Sprintf("Exspected %d arguments but got %d", function.Arity(), len(args)))
		}

		return function.Call(args)
	case *ast.Function:
		function := NewUserFunction(n, environment, res, n.EnvSize)
		environment.Define(n.Name.Lexeme, function, n.EnvIndex, "void")
		return nil, nil
	case *ast.Return:
		var value interface{}
		var err error
	...
	case *ast.Class:
		...
		environment.Define(n.Name.Lexeme, nil, n.EnvIndex, "void")

		if superclass != nil {
			environment = env.NewSized(environment, 1)
			environment.Define("super", superclass, 0, "void")
		}
